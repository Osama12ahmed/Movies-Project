...
export default function DetailsS() {
    ...
    useEffect(() => {
        const fetchShow = async () => {
            try {
                ...
                const showDetails = await detailsRes.json();
                ...
                setShow(showDetails);
                setGenres(showDetails.genres.map((g) => g.name).join(", "));
                setRating(showDetails.vote_average?.toFixed(1));
    ...
    return (
        <>
            ...
            <DetailsSV title={title} apiKey={apiKey} movie={show}/>
            <Actors cast={cast} />
            <Footer/>
        </>
    );
